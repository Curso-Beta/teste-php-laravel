...
<body>
   ...
   <div class="container p-5 m-5 text-center">
       <h1>Lista de vagas 
         ...
         <span><a  class="btn btn-outline-success" href="{{ route('adicionar.vaga')}}" >Criar vaga</a></span>
       </h1>       
   </div>
   ...
   <table class="table">
       ...
       <tbody>
          <tr>
            ...
            <td>{{$vaga->status}}</td>  
            <td>{{$vaga->salario}}</td>       
            <td><a class="btn btn-outline-info" href="{{route('editar.vaga',$vaga->id)}}">Editar</a></td>  
            <td><a class="btn btn-outline-danger" href="{{route('excluir.vaga',$vaga->id)}}">Excluir</a></td>
          </tr>  
       </tbody>
      @endforeach
    </table>            
